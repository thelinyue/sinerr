...
* **Emby 账号创建:** 管理员创建本地用户时可勾选「同时创建 Emby 账号」，自动在 Emby 服务器创建同名账号并同步模板用户权限策略
* **模板用户:** Emby 设置中可选择模板用户，创建新 Emby 账号时自动复制其权限设置（排除管理员权限）
* **自动生成密码:** 无需配置邮箱即可使用自动生成密码功能，创建完成后弹窗展示用户名和密码，支持逐项复制和全部复制
* **连接信息:** 右上角头像下拉菜单新增「连接信息」入口，统一展示服务器地址（主机+端口分离显示）和客户端下载链接，支持一键复制
* **客户端下载配置:** 设置中可配置多个客户端下载入口（图标/名称/下载地址），支持 emoji 和图片 URL 图标，实时预览
* **首页引导卡片:** 探索页顶部新增可关闭的连接引导卡片，分步骤展示下载客户端 → 输入服务器地址 → 登录 Emby，含可视化连接示例截图
* **端口号提示:** 连接示例和弹窗中单独展示端口号字段，默认 443，提醒用户检查端口设置
* **用户凭据弹窗:** 创建 Emby 账号成功时弹窗额外展示 Emby 服务器地址和登录说明


### Removed

* **YouTube URL:** 移除设置中的 YouTube URL 配置项，预告片默认使用 `https://www.youtube.com/watch?v=`
* **预告片功能:** 彻底移除预告片相关功能，取消 TMDB API 的 `videos` 数据请求，减少接口调用和带宽消耗
* **Emby 公开注册:** 移除公开的 Emby 注册页面，注册功能改为管理员创建用户时可选
* **Emby 密码重置:** 移除 Emby 密码重置路由和页面
