...
description: How to configure Sinerr to work with services that use self-signed SSL certificates.
---
...
If your media server or services (Radarr, Sonarr, etc.) use self-signed SSL certificates, Sinerr will reject the connection because it does not trust them by default. The fix is to add your CA certificate to Node.js.
...
:::tip
The certificate must be in PEM format. Open it in a text editor â€?if it starts with `-----BEGIN CERTIFICATE-----`, it is PEM. If it contains binary data, convert it with `openssl x509 -inform DER -in cert.cer -out cert.pem`.
:::
