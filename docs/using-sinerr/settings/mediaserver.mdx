...
In this section, simply select the libraries you would like Sinerr to scan. Sinerr will periodically check the selected libraries for available content to update the media status that is displayed to users.
...
Sinerr will perform a full scan of your Jellyfin libraries once every 24 hours (recently added items are fetched more frequently). If this is your first time configuring Jellyfin, a one-time full manual library scan is recommended!
...
The internal URL is the URL that Sinerr will use to communicate with your Jellyfin server. This URL should be accessible from the machine running Sinerr.
...
:::note
When running Sinerr in a docker container with a bridged network (default), the container's network will be separate from the host network. Therefore, you cannot use `localhost` or `127.0.0.1` as the internal URL as it will resolve to the container itself.
:::
:::tip
If you are running Jellyfin in a docker container, you can put both Sinerr and Jellyfin on the same docker network by using a custom [docker network](https://docs.docker.com/reference/cli/docker/network/). This will allow you to use the container name as the internal URL.
:::
...
The forgot password URL is the URL that users will be directed to when they click the "Forgot Password" button on the login page. This URL should be accessible from the machine running Sinerr.
...
If you have Sinerr installed on the same network as Jellyfin, you can set this to the local IP address of your Jellyfin server. Otherwise, this should be set to a valid hostname (e.g., jellyfin.myawesomeserver.com).
...
Enable this setting to connect to Jellyfin via HTTPS rather than HTTP. Self-signed certificates are not trusted by default, but you can configure Sinerr to accept them. See [Self-Signed Certificates](/using-sinerr/advanced/self-signed-certificates) for details.
...
Sinerr will perform a full scan of your Emby libraries once every 24 hours (recently added items are fetched more frequently). If this is your first time configuring Emby, a one-time full manual library scan is recommended!
...
The internal URL is the URL that Sinerr will use to communicate with your Emby server. This URL should be accessible from the machine running Sinerr.
...
:::
:::tip
If you are running Emby in a docker container, you can put both Sinerr and Emby on the same docker network by using a custom [docker network](https://docs.docker.com/reference/cli/docker/network/). This will allow you to use the container name as the internal URL.
:::
...
If you have Sinerr installed on the same network as Emby, you can set this to the local IP address of your Emby server. Otherwise, this should be set to a valid hostname (e.g., jellyfin.myawesomeserver.com).
...
Enable this setting to connect to Emby via HTTPS rather than HTTP. Self-signed certificates are not trusted by default, but you can configure Sinerr to accept them. See [Self-Signed Certificates](/using-sinerr/advanced/self-signed-certificates) for details.
...
Depending on your setup/configuration, you may need to enter your Plex server details manually in order to establish a connection from Sinerr.
:::

#### Hostname or IP Address

If you have Sinerr installed on the same network as Plex, you can set this to the local IP address of your Plex server. Otherwise, this should be set to a valid hostname (e.g., `plex.myawesomeserver.com`).
...
Enable this setting to connect to Plex via HTTPS rather than HTTP. Self-signed certificates are not trusted by default, but you can configure Sinerr to accept them. See [Self-Signed Certificates](/using-sinerr/advanced/self-signed-certificates) for details.
...
Sinerr will perform a full scan of your Plex libraries once every 24 hours (recently added items are fetched more frequently). If this is your first time configuring Plex, a one-time full manual library scan is recommended!
