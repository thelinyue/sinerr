...
description: Install Sinerr using Unraid
sidebar_position: 2
---
...
**Sinerr Default** (`UID 1000:1000`) â€?Runs with the container's native `node` user. Matches the official image, but may break SMB share access, trigger "Fix Common Problems" warnings, and cause issues with backup plugins (such as CA Appdata Backup) and network integrations (such as Tailscale).

**Unraid Default** (`UID 99:100`) â€?Runs with Unraid's native `nobody:users` ownership. Follows Unraid best practices, but overrides Sinerr's built-in rootless user and may cause compatibility issues with future Sinerr updates.

<Tabs groupId="unraid-permissions" queryString>
  <TabItem value="sinerr-default" label="Sinerr Default">
  ...
  ```bash
  mkdir -p /mnt/user/appdata/sinerr
  ```

  ### 2. Set folder permissions

  ```bash
  chown -R 1000:1000 /mnt/user/appdata/sinerr
  ```
  ...
  |---|---|
  | **Name** | `sinerr` |
  | **Repository** | `ghcr.io/thelinyue/sinerr:latest` |
  | **Icon URL** | `https://raw.githubusercontent.com/thelinyue/sinerr/develop/public/android-chrome-512x512.png` |
  ...
  **Path:**
  ...
  | Host Path | `/mnt/user/appdata/sinerr` |
  ...
  **Variable (optional):**
  ...
  ```bash
  chown -R 99:100 /mnt/user/appdata/sinerr
  ```
