...
**Sinerr Default** (`UID 1000:1000`) �?Runs with the container's native `node` user. Matches the official image, but may break SMB share access, trigger "Fix Common Problems" warnings, and cause issues with backup plugins (such as CA Appdata Backup) and network integrations (such as Tailscale).

**Unraid Default** (`UID 99:100`) �?Runs with Unraid's native `nobody:users` ownership. Follows Unraid best practices, but overrides Sinerr's built-in rootless user and may cause compatibility issues with future Sinerr updates.
...
<Tabs groupId="unraid-permissions" queryString>
  ...
  |---|---|
  | **Name** | `sinerr` |
  | **Repository** | `linyuedoc/sinerr:latest` |
